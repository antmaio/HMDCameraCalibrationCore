"""
config.py
Central configuration for the pose capture pipeline.
Edit this file to match your hardware setup.
"""

# ------------------------------------------------------------------
# ZED Camera Settings
# ------------------------------------------------------------------

# Replace with your actual ZED serial numbers
CAMERA_SERIAL_NUMBERS: list[int]    = [
    35429635,
    38579233 #TODO: Replace with real serial numbers after testing each camera individually
    #33333333, #TODO: Replace with real serial numbers after testing each camera individually
]

CAMERA_FPS:int                      = 60

#For calibration 
CALIBRATION_BOARD_SIZE:tuple        = (5, 6)  # Number of inner corners per chessboard row and columnchs
SQUARE_SIZE:float                   = 0.25  # Size of a square in meters

# ------------------------------------------------------------------
# OSC / Unity Settings
# ------------------------------------------------------------------

#HMD_OSC_IP: str = "10.104.202.34"  #standalone mode
HMD_OSC_IP:str                      = "127.0.0.1" #PCVR mode
SEND_OSC_PORT:int                   = 5005
LISTEN_OSC_PORT:int                 = 5010
OSC_TARGET_HZ:float                 = 60.0

# ------------------------------------------------------------------
# YOLO Settings
# ------------------------------------------------------------------

#YOLO_WEIGHTS: str = "yolo_model/yolov8n-pose/train_asymscale150/weights/last"  # Path to your YOLO pose model weights
YOLO_WEIGHTS:str                    = "yolo_model/baseline/engine/fp32/yolov8x-pose"  # Path to your YOLO pose model weights
#YOLO_WEIGHTS:str                = "./yolo_model/yolov8x-pose/train_asymcale150/weights/last"  # Path to your YOLO pose model weights
YOLO_FORMAT:str                     = "engine"  # Choose one of: 'pt', 'onnx', or 'engine'
IMAGE_TARGET_SIZE:int               = 640  # Image target size after resize (letterbox)
N_COCO_KEYPOINTS:int                = 17  # Number of keypoints in the COCO format
MIN_KEYPOINT_CONFIDENCE:float       = 0.5 # below this, a joint's triangulated 3D position is zeroed out


def valid_format(YOLO_FORMAT: str) -> tuple[bool, bool]:
    yolo_format_choices: tuple[str, ...] = ("pt", "onnx", "engine")
    if YOLO_FORMAT.lower() not in yolo_format_choices:
        raise ValueError(
            f"YOLO_FORMAT must be one of {yolo_format_choices}; got {YOLO_FORMAT!r}"
        )
    use_onnx: bool = YOLO_FORMAT == "onnx"
    use_engine: bool = YOLO_FORMAT == "engine"
    return use_onnx, use_engine
USE_ONNX, USE_ENGINE                = valid_format(YOLO_FORMAT)


#COCO keypoint indices:
# 0: Nose
# 1: Left Eye
# 2: Right Eye
# 3: Left Ear
# 4: Right Ear
# 5: Left Shoulder
# 6: Right Shoulder
# 7: Left Elbow
# 8: Right Elbow
# 9: Left Wrist 
# 10: Right Wrist
# 11: Left Hip
# 12: Right Hip
# 13: Left Knee
# 14: Right Knee
# 15: Left Ankle
# 16: Right Ankle

# Coco keypoint order for reference
SKELETON_EDGES:list[tuple[int, int]] = [
    (15, 13), (13, 11), (16, 14), (14, 12), (11, 12), 
    (5, 11), (6, 12), (5, 6), (5, 7), (6, 8), (7, 9), 
    (8, 10), (1, 2), (0, 1), (0, 2), (1, 3), (2, 4), 
    (3, 5), (4, 6)
]

